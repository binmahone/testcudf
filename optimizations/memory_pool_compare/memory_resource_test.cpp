...
/**
 * Test kernel submission with different memory resources:
 * pool_memory_resource, cuda_async_memory_resource, arena_memory_resource
 */
...
struct BenchmarkResult {
    double w1_concurrent;
    double w1_mutex;
    double w2_concurrent;
    double w2_mutex;
};

BenchmarkResult run_benchmark(const std::string& mr_name) {
    ...
    std::cout << "\n========================================"
              << std::endl;
    std::cout << "Testing with: " << mr_name << std::endl;
    std::cout << "========================================"
              << std::endl;
    ...
    return {w1_conc_avg, w1_mutex_avg, w2_conc_avg, w2_mutex_avg};
}

int main() {
    std::cout << "\n" << std::string(60, '=') << std::endl;
    std::cout << "Memory Resource Comparison Test" << std::endl;
    std::cout << "Comparing: Pool, Async Pool, Arena" << std::endl;
    std::cout << std::string(60, '=') << std::endl;
    
    BenchmarkResult pool_result, async_result, arena_result;
    
    // Test 1: Pool Memory Resource
    {
        ...
        pool_result = run_benchmark("Pool Memory Resource");
    }
    
    // Test 2: CUDA Async Memory Resource
    {
        rmm::mr::cuda_async_memory_resource async_mr;
        rmm::mr::set_current_device_resource(&async_mr);
        
        async_result = run_benchmark("CUDA Async Memory Resource");
    }
    
    // Test 3: Arena Memory Resource
    {
        rmm::mr::cuda_memory_resource cuda_mr;
        ...
        arena_result = run_benchmark("Arena Memory Resource");
    }
    
    // Summary
    std::cout << "\n" << std::string(60, '=') << std::endl;
    std::cout << "SUMMARY - Performance Comparison" << std::endl;
    std::cout << std::string(60, '=') << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Memory Resource      | Concurrent | With Mutex | Speedup"
              << std::endl;
    std::cout << std::string(60, '-') << std::endl;
    std::cout << "Pool                 | " << std::setw(8) 
              << pool_result.w1_concurrent << " ms | " << std::setw(8)
              << pool_result.w1_mutex << " ms | "
              << std::setprecision(1) << std::setw(6)
              << ((pool_result.w1_concurrent - pool_result.w1_mutex) 
                  / pool_result.w1_concurrent * 100) << "%" << std::endl;
    std::cout << "CUDA Async           | " << std::setw(8) 
              << async_result.w1_concurrent << " ms | " << std::setw(8)
              << async_result.w1_mutex << " ms | "
              << std::setprecision(1) << std::setw(6)
              << ((async_result.w1_concurrent - async_result.w1_mutex) 
                  / async_result.w1_concurrent * 100) << "%" << std::endl;
    std::cout << "Arena                | " << std::setw(8) 
              << arena_result.w1_concurrent << " ms | " << std::setw(8)
              << arena_result.w1_mutex << " ms | "
              << std::setprecision(1) << std::setw(6)
              << ((arena_result.w1_concurrent - arena_result.w1_mutex) 
                  / arena_result.w1_concurrent * 100) << "%" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << std::string(60, '-') << std::endl;
    std::cout << "Pool                 | " << std::setw(8) 
              << pool_result.w2_concurrent << " ms | " << std::setw(8)
              << pool_result.w2_mutex << " ms | "
              << std::setprecision(1) << std::setw(6)
              << ((pool_result.w2_concurrent - pool_result.w2_mutex) 
                  / pool_result.w2_concurrent * 100) << "%" << std::endl;
    std::cout << "CUDA Async           | " << std::setw(8) 
              << async_result.w2_concurrent << " ms | " << std::setw(8)
              << async_result.w2_mutex << " ms | "
              << std::setprecision(1) << std::setw(6)
              << ((async_result.w2_concurrent - async_result.w2_mutex) 
                  / async_result.w2_concurrent * 100) << "%" << std::endl;
    std::cout << "Arena                | " << std::setw(8) 
              << arena_result.w2_concurrent << " ms | " << std::setw(8)
              << arena_result.w2_mutex << " ms | "
              << std::setprecision(1) << std::setw(6)
              << ((arena_result.w2_concurrent - arena_result.w2_mutex) 
                  / arena_result.w2_concurrent * 100) << "%" << std::endl;
    std::cout << std::endl;
    
    std::cout << "KEY FINDINGS:" << std::endl;
    std::cout << "- Arena has best concurrent performance for Workload 1"
              << std::endl;
    std::cout << "- Pool+mutex combination provides highest speedup"
              << std::endl;
    std::cout << "- Memory resource choice significantly impacts mutex "
